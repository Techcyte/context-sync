...
The demo host is a `go` project so you will need to have go installed in order to use it. Go can be installed from [here](https://go.dev/doc/install).
...
The demo host is a TUI app. Press `n` to input a new case number then press `enter` to send a context change request. Press `c` to clear the console. Press `q` to quit.
