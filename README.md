...
* `context` - An array of context objects. Optional for `sync-accept`, and `ctx-update` messages, where omission indicates no current context. Required for `ctx-change-request` messages.
...
* `current_context` - The current context. Required for `ctx-change-reject` messages. Can only be used with `ctx-change-reject` messages.
	* `key: string` - The context kind.
	* `value: string` - The context value.
* `error` - For unexpected errors. Optional for `ctx-update` messages. Can only be used in `ctx-update` messages.
...
Sent unilaterally or by request by either the server or the client to notify the other party of their current context state. This is
used in several scenarios:

1. **Empty context** - When the sender has no current context (e.g., user navigates to worklist), send with no context field or empty array:
```JSON
{
  "kind": "ctx-update"
}
...
2. **Post-accept failure** - After accepting a context change request but failing to navigate to the new context. Example: both parties
agreed to switch to a case, but after sending `ctx-change-accept`, the server fails to switch due to an unexpected
internal error. Then the server sends `ctx-update` with its current context, and the client and server show error messages.

3. **After rejected request** - When a party sends a `ctx-change-request` that gets rejected, but the requesting party has
already navigated to the new context or chooses to navigate anyway. The requesting party sends `ctx-update` to inform the other of the desynchronization.

4. **Unexpected message received** - When a party receives an unexpected or invalid message. For example if an active client receives
a `ctx-change-request` message with no context the client would respond with:
...
```JSON
{
  ...
  "error": {
    "message": "ctx-change-request sent with no context.",
    "status": 400
  }
}
```
...
```JSON
{
  "kind": "ctx-update-request"
}
```

Can be used to re-sync the context. For example the user in Fusion could click a button to re-sync with the server and
send a `ctx-update-request` message to ensure both the client and the server have the same context.
...
sequenceDiagram
    ...
    Client->>Server: ctx-update (no context)
```
...
sequenceDiagram
    ...
    Note over Server: Show out of sync error to user
    Server->>Client: ctx-update (current context)
    Note over Client: Show out of sync error to user
```
...
sequenceDiagram
    ...
    Client->>Server: sync-request (connection info)
    Server->>Client: sync-accept (connection info)
    Note over Client: User clicks on new case (A)
    Note over Server: User clicks on new case in LIS (B)
    ...
    alt Server accepts client's request
        ...
        Note over Server: Navigate to context A
        ...
        Note over Client,Server: Show out of sync errors to user
    end
```
